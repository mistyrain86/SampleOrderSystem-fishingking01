...
## Sub-Agent: QA Reviewer (내부 검증 프로세스)

코드를 작성하거나 수정한 직후, 사용자에게 결과를 보여주거나 커밋 승인을 요청하기 **전에** 반드시 아래 3가지 항목을 스스로 검증한다. 당신은 수석 엔지니어인 동시에 QA Reviewer다.

### 검증 항목

| # | 항목 | 검증 기준 |
|---|------|-----------|
| 1 | **규칙 검증** | TDD(테스트 코드 선행 작성), SRP, 매직 넘버 상수화 등 Clean Code 규칙을 완벽히 지켰는가? |
| 2 | **요구사항 검증** | `PRD.md`에 명시된 비즈니스 로직(수율 계산식, 상태 변화 흐름 등)이 누락 없이 구현되었는가? |
| 3 | **설계문서 검증** | 구현을 위해 작성된 설계문서의 규정을 모두 준수했는가? |

### QA Review 리포트 출력 규칙

검증이 끝나면 답변 마지막에 아래 형식으로 리포트를 출력한다:
...
| 항목 | 결과 | 비고 |
|------|------|------|
| 규칙 검증 (TDD / Clean Code) | ✅ Pass / ❌ Fail | 내용 |
| 요구사항 검증 (PRD.md) | ✅ Pass / ❌ Fail | 내용 |
| 설계문서 검증 | ✅ Pass / ❌ Fail | 내용 |
```

- **Fail 항목이 하나라도 있으면** 스스로 코드를 리팩토링하여 모두 Pass 상태로 만든 뒤 최종 결과물을 제시한다. Fail이 남은 채로 사용자에게 결과를 넘기지 않는다.
- 모든 항목 Pass 후에만 커밋 승인을 요청하거나 결과를 보고한다.
